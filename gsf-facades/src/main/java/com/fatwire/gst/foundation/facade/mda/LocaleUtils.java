...
public final class LocaleUtils
{
     ...
     * <p/>
     * If a dimension set for the site has been configured that returns the asset other than the preferred locale,
     * that is considered to be fine and not really the problem of the end user.  In other words, a dimension set
     ...
     * @deprecated Use #findTranslation(ICS,String,String,String,String) instead.  This version is unable to
     *             correctly record compositional dependencies on the DimensionSet asset for the site, which can result
     *             in the translation function not flushing pages when the dimension set is altered (for example, to
     *             change the filter, or change enabled dimensions).
     */
     ...
     * @param site name of site
     * @return AssetId of translation asset, or null if none is returned by the dimension set filter.
     * @deprecated Use #findTranslation(ICS,AssetId,String,String) instead.  This version is unable to
     *             correctly record compositional dependencies on the DimensionSet asset for the site, which can result
     *             in the translation function not flushing pages when the dimension set is altered (for example, to
     *             change the filter, or change enabled dimensions).
     */
    ...
    public static AssetId findTranslation(ICS ics, AssetId id, long preferredDimension, long dimensionSetId)
    {
        ...
        // *****************************************************************************
        // The core business logic of this helper class is encapsulated in these 3 lines
        ...
        else
        {
            switch(relatives.size())
            {
                case 0:
                {
                    ...
                    // Note May 4, 2010 by Tony Field - this had been changed to return the input ID but that
                    // is incorrect.  The contract clearly states that null is to be returned if no matching
                    // relatives are found.  When null is returned and it is not expected, often the incorrect
    ...
    /**
     * Locates a single dimension set in a site.  If no match is found, an exception is thrown.  If more than one match
    ...
    private static DimensionFilterInstance _getPopulatedDimensionFilter(Session ses, DimensionSetInstance dimset, long localeDimensionId)
    {
        ...
        //        Equivalent to:
        //           %><dimensionset:asset assettype="Dimension" assetid="<%=localeDimensionId%>" /><%
