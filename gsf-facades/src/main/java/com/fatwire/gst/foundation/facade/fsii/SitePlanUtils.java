...
import com.fatwire.gst.foundation.facade.runtag.asset.GetSubtype;
 ...
 * Placeholders are pages that are typically located at the root of the site
 * plan tree and do not contain any content. They are not content assets but are
...
public final class SitePlanUtils {
     ...
     * asset. Note that the asset type of the page detail is not fixed - any one
     * will work.
     * 
     * @param ics context
     * @param p id of page asset
     * @param assocname name of the association between the page and the page
     *            detail asset
     ...
     * @param c asset type
     * @param cid asset id
     * @param metaKeywordAttribute attribute used to contain meta keywords. must
     *            be multi-valued
     * @param metaDescriptionAttribute attribute used to contain description.
     *            expects single-valued
     * @param titleAttribute attribute used to contain title. expects
     *            single-valued.
     * @return HeadTagData object
     */
    public static HeadTagData getStandardHeadData(ICS ics, String c, String cid, String metaKeywordAttribute,
     ...
     * @param ics context
     * @param p page
     * @param assocname association name for pageDetail if applicable. May be
     *            null
     * @param linktextAttr attribute in associated pagedetail if specified,
     *            otherwise attribute in page asset
     * @param tname Template name to link to. Required if links are to work but
     *            can be omitted if links are not desired.
     ...
     * @return PageLinkData
     */
    public static PageLinkData getSitePlanAsLinks(ICS ics, final String p, String assocname, String linktextAttr,
            String tname, String wrapper, boolean withChildren) {
            ...
            try {
                ...
                if (e.getErrno() == -111) {
                    ...
                    if (!isNavigationPlaceholder) {
                        throw new CSRuntimeException("Missing required association: " + assocname + " for Page:"
                                + resolvePageAlias(ics, p), -111);
                    }
        ...
        if (!isNavigationPlaceholder && tname != null) {
            ...
            if (tname != null) {
                ...
                gtu.setArgument("p", p); // should this include the alias
                                         // target?
        ...
        if (withChildren) {
            for (AssetId kid : ListPages.getChildPages(ics, pageLinkData.getPageId().getId())) {
                // recurse
                PageLinkData kidData = getSitePlanAsLinks(ics, Long.toString(kid.getId()), assocname, linktextAttr,
                        tname, wrapper, withChildren);
     ...
     * return the ID of the target page. If not an alias, the original input is
     * returned
     * 
     * @param ics context
