...
public class Children extends AbstractTagRunner {
	 ...
	 */
	public static AssetId getSingleAssociation(ICS ics, String c, String cid,
			String code) {
		...
		if (spukids != null && spukids.hasData()) {
			if (spukids.numRows() > 1) {
				throw new IllegalStateException(
						"Too many kids found associated to: " + c + ":" + cid
								+ " for association:" + code
								+ ".  expected 1 but got " + spukids.numRows());
			}
			spukids.moveTo(1);
			return new AssetIdImpl(IListUtils.getStringValue(spukids, "otype"),
					IListUtils.getLongValue(spukids, "oid"));
		} else {
			throw new IllegalStateException("No kids found associated to: " + c
					+ ":" + cid + " for association:" + code
	 ...
	 */
	public static List<AssetId> getOptionalMultivaluedAssociation(ICS ics,
			String c, String cid, String code) {
		ics.RegisterList("spu-kids", null);
		Children ac = new Children();
		ac.setType(c);
		ac.setAssetId(cid);
		...
		if (spukids != null && spukids.hasData()) {
			for (IList row : new IterableIListWrapper(spukids)) {
				result.add(new AssetIdImpl(IListUtils.getStringValue(row,
						"otype"), IListUtils.getLongValue(row, "oid")));
	 ...
	 * @return ids of the associated asset
	 */
	public static List<AssetId> getMultivaluedAssociation(ICS ics, String c,
			String cid, String code) {
		List<AssetId> result = getOptionalMultivaluedAssociation(ics, c, cid,
				code);
		if (result.size() == 0) {
			throw new IllegalStateException("No kids found associated to: " + c
					+ ":" + cid + " for association:" + code
					+ ".  expected at least one but got none.");
		}
		return result;
	}
}
