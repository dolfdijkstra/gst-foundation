...
public class BaseController extends AbstractController
{
    ...
    protected void doExecute()
    {
        ...
        String site = resolveSite(id); // todo: optimization - store in GSTUrlRegistry
        if(site == null)
        {
    ...
    protected AssetId resolveAssetId()
    {
        ...
        else if(goodString(ics.GetVar("c")) && goodString(ics.GetVar("cid")))
        {
            ...
            id = new AssetIdImpl(ics.GetVar("c"), Long.parseLong(ics.GetVar("cid")));
        }
    ...
    protected String resolveSite(AssetId webReferenceableAssetId)
    {
        ...
        param.setString(0, webReferenceableAssetId.getType());
        param.setLong(1, webReferenceableAssetId.getId());
        String result = null;
        for(Row pubid : SqlHelper.select(ics, stmt, param))
        {
            if(result != null)
            {
                LOG.warn("Found asset " + webReferenceableAssetId + " in more than one publication. It should not be shared; aliases are to be used for cross-site sharing.  Controller will use first site found");
            }
            else
            {
    ...
    protected AssetId resolveAssetFromUrl(final String virtual_webroot, final String url_path)
    {
    ...
    protected void callTemplate(final String site, final AssetId id, final String tname)
    {
        ...
        ct.setTname(tname);
        ct.setContext("");
        String target = tname.startsWith("/") ? site + "/" + tname : site + "/" + id.getType() + "/" + tname; //typeless or not
    ...
    protected String getCallTemplateCallStyle(String target)
    {
        if(ics.isCacheable(ics.GetVar(ftMessage.PageName)))
        {
            // call as element when current is caching.
            // todo: optional: call embedded to allow nested page to be cached separately
            // to allow for pre-caching based on startdate/enddate, or for security
            // reasons (e.g. cache with criteria validated, but call from controller would happen conditionally
            // Need to think about perf. costs of using "embedded" here. If negligible, consider
