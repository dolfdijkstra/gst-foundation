...
public abstract class AbstractController implements Seed2 {
    ...
    /*
     * (non-Javadoc)
     * @see COM.FutureTense.XML.Template.Seed#Execute(COM.FutureTense.Interfaces.FTValList, COM.FutureTense.Interfaces.FTValList)
     */
    ...
    protected String sendError(final int code, final Exception e) {
        LOG.debug(code + " status code sent due to exception " + e.toString(),
                e);
        switch (code) { //all the http status codes, we may restrict the list to error and redirect
        case 100:
        case 101:
        case 200:
        case 201:
        ...
        Render.UnknownDeps(ics);//failure case might be corrected on next publish or save
        String element = null;
        ...
        ics.SetErrno(-500); // or whatever make sense
